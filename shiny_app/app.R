...
server <- function(input, output, session) {
  ...
  ensembl <- eventReactive(input$submit,
         {
           snps <- NULL
           if (!is.null(input$text) && nzchar(input$text)) {
             raw_data <- unlist(strsplit(trimws(input$text), "[,\t\n]+"))
             
             # Filter to keep only the SNPs starting with 'rs'
             snps <- raw_data[startsWith(raw_data, "rs")]
           }
           
           # Handle file upload
           if (!is.null(input$upload)) {
             df <- data()
             snps <- unique(unlist(df))  # Flatten and remove duplicates
           }
           e <- ensembl_sql(snps)
  ...
  #    })
  # 
  # output$hgnc <- DT::renderDT(server = TRUE, {
  #   req(normalize())
  #   normalize()$data
  # })
  # 
  # output$omim <- DT::renderDT(server = TRUE, {
  #   req(omim_set())
  #   omim_set()
  # })
  # 
  ...
  # })
  # 
  # output$gsea <- DT::renderDT(server = TRUE, {
  #   req(gene_set())
  #   gene_set()
  # })
  # 
